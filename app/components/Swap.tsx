...
export function Swap({ publicKey, tokenBalances }: {
    ...
    return <div className="p-12">
        ...
        subtitle={<div className="text-slate-500 pt-1 text-sm pl-1">Current Balance: {tokenBalances?.tokens.find(x => x.name === baseAsset.name)?.balance} {baseAsset.name}</div>} />
...
function SwapInputRow({ onSelect, selectedToken, title, subtitle, topBorderEnabled, bottomBorderEnabled }: {
    ...
    return <div className={`border flex justify-between p-4 ${topBorderEnabled ? "rounded-t-xl" : ""} ${bottomBorderEnabled ?
        "rounded-b-xl" : ""} `}>
        <div>
